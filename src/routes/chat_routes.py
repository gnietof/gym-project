...
from services.chat_services import ask_agentic, ask_question, score_answer
...
async def ask(payload: ChatRequestDTO, db: Session = Depends(get_db)):
    ...
    answer, track = await ask_question(id, question, db)
    response = ChatResponseDTO(id=id, answer=answer, track=track)
    return response


@router.post("/askAgent", response_model=ChatResponseDTO)
async def askAgent(payload: ChatRequestDTO, db: Session = Depends(get_db)):
    id = payload.id
    if not id:
        id = str(uuid.uuid4())
