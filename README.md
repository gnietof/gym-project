...
| Rubric item | Concrete evidence |
| - | - |
| Problem description | The problem has been described at the [top of the document](#gym-assistant) and in the [Problem](#problem) section.|
| Retrieval flow | An agent retrieves the information using two different [AI tools](src/ai_tools): one uses semantic search and the other uses the LLM to generate SQL queries. Once the agent has enough information the LLM generates an [aswer](src/services/chat_services.py#:~:text=def%20ask_agentic).
| Retrieval evaluation | Semantic retrieval evaluation is included [here](#retrieval-evaluation) 
| LLM evaluation | Different LLM's where used to evaluate the responses to a collection of questions. The evaluation is included [here](#rag-evaluation) and the details are in [this folder](src/evaluations) |
...
| Containerization | The solution has been containerized using two containers. One for FastAPI and one for Postgres. The [Dockerfile](Dockerfile) for FastAPI and the [docker-compose.yml) have been included in the repository. The repository also includes a database backup which is automatically restored in the database when the Postgres container is first launched.
| Reproducibility | All the required data and code are provided and instructions have been included.
| Hybrid Search | While keyword search has been evaluated, it has not been included because its poor performance. Semantic search and SQL queries (being generated by the LLM) to retrieve information from the database have been used instead.
...
| Extra evidence | A demo has been included which displays several type of questions which involve one or both tools (and one or both search methods). | 
